...
func TestWildcardsInfo(t *testing.T) {
	...
	}
	{
		...
		assert.False(wildcards.ResultCol2WildcardCol(0).Valid())
		assert.Equal("id", wildcards.ResultCol2WildcardCol(1).ColumnName())
		assert.Equal("name", wildcards.ResultCol2WildcardCol(2).ColumnName())
		assert.Equal("female", wildcards.ResultCol2WildcardCol(3).ColumnName())
		assert.False(wildcards.ResultCol2WildcardCol(4).Valid())
		assert.Equal("id", wildcards.ResultCol2WildcardCol(5).ColumnName())
		assert.Equal("name", wildcards.ResultCol2WildcardCol(6).ColumnName())
		assert.Equal("female", wildcards.ResultCol2WildcardCol(7).ColumnName())
		assert.False(wildcards.ResultCol2WildcardCol(8).Valid())
